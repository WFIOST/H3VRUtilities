...
using System.Text;
using FistVR;
using H3VRUtils.NewScripts;
using UnityEngine;

namespace H3VRUtils
{
	public class FieldGun : FVRFireArm
	{
		public FieldGunRotatingLever _xRotation;
		private float _xlerp = 2;
		public FieldGunRotatingLever _yRotation;
		private float _ylerp = 2;

		public float maxXrotDeviance;
		
		public float maxYrotDeviance;
		...
		public override void FVRUpdate()
		{
			...
			//rotate the rots
			if (_xRotation.lerp != _xlerp)
			{
				_xlerp = _xRotation.lerp;
				SetRotationOfAxis(0, maxXrotDeviance, _xlerp);
			}
			if (_yRotation.lerp != _ylerp)
			{
				_ylerp = _yRotation.lerp;
				SetRotationOfAxis(0, maxYrotDeviance, _ylerp);
			}
		}

		private void SetRotationOfAxis(int axis, float maxRotDeviance, float lerpAmt)
		{
			bool isNeg = lerpAmt < 0;
			var rot = Mathf.Lerp(0, maxRotDeviance, lerpAmt);
			if (isNeg) rot *= -1;
