...
public class VehicleControl : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        wheels[2] = SetWheelComponent(carWheels.wheels.backRight, 0, carWheels.wheels.backWheelDrive, carWheels.wheels.backRight.position.y, carWheels.wheels.rearSetting);
        wheels[3] = SetWheelComponent(carWheels.wheels.backLeft, 0, carWheels.wheels.backWheelDrive, carWheels.wheels.backLeft.position.y, carWheels.wheels.rearSetting);
        ...
        foreach (WheelComponent w in wheels)
        {
            ...
            col.suspensionDistance = w.settings.Distance;
            JointSpring js = col.suspensionSpring;

            js.spring = carSetting.springs;
            js.damper = carSetting.dampers;
            col.suspensionSpring = js;
            col.radius = w.settings.Radius;
            col.mass = w.settings.Weight;
    ...
    public void ShiftTo(int newGear)
    {
        ...
        if (currentGear == 0) NeutralGear = true;
        else NeutralGear = false;
    ...
    public void ShiftUp(bool ignoreDelay)
    {
        ...
        if (now < shiftDelay && !ignoreDelay) return;
    ...
    public void ShiftDown(bool ignoreDelay)
    {
       ...
       if (now < shiftDelay && !ignoreDelay) return;
        ...
        {
            //TODO: Why the fuck does this call a getcomponent??? cache this shit!
            //same with ShiftUp ofc
    ...
    void FixedUpdate()
    {
        ...
        if (carSetting.carSteer) carSetting.carSteer.localEulerAngles = new Vector3(steerCurAngle.x, steerCurAngle.y, steerCurAngle.z + (steer * -120.0f));

        if (carSetting.automaticGear && (currentGear == 1) && (accel < 0.0f)) {
            if (speed < 5.0f) ShiftDown(false); }
        else if (carSetting.automaticGear && (currentGear == 0) && (accel > 0.0f)) {
            if (speed < 5.0f) ShiftUp(false); }
    ...
    : slip = Mathf.Lerp(slip, 0.01f, 0.02f);
            ...
            else
            {
                ...
                lp.y = w.startPos.y - w.settings.Distance;
                myRigidbody.AddForce(Vector3.down * 5000);
            }
            currentWheel++;
            w.wheel.localPosition = lp;
        }
        if (motorizedWheels > 1) rpm = rpm / motorizedWheels;
        ...
        foreach (WheelComponent w in wheels)
        {
            ...
            if (w.drive)
            {
                if (Mathf.Abs(col.rpm) > Mathf.Abs(wantedRPM)) { col.motorTorque = 0; }
                else
                {
        ...
        if (!isOn)
        {
            carSounds.IdleEngine.volume = 0;
            carSounds.LowEngine.volume = 0;
            carSounds.HighEngine.volume = 0;
        }
